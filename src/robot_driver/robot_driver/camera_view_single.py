...
class CameraCaptureROS(Node):
    ...
    def _declare_and_load_params(self):
        ...
        self.declare_parameter('camera_count', 3)
    ...
    def _signal_handler(self, signum, frame):
        self.get_logger().debug(f"Signal {signum} received, stopping capture...")
        self.running = False
        try:
            rclpy.shutdown()
        except Exception:
    ...
    def _camera_grab_loop(self, cam):
        while self.running and rclpy.ok():
            try:
                ...
                self.get_logger().warn(f"Camera {cam['id']} grab loop error: {e}")
                time.sleep(0.01)
    ...
    def _start_grab_threads(self):
        ...
        for cam in self.cameras:
            t = threading.Thread(
                target=self._camera_grab_loop,
                args=(cam,),
                name=f"camera_grab_{cam['id']}",
                daemon=True,
            )
            self._grab_threads.append(t)
            t.start()

    def _stop_grab_threads(self):
        self.running = False
        for t in getattr(self, '_grab_threads', []):
            if t.is_alive():
                t.join(timeout=3)
