#!/usr/bin/env python3
# ROS2 port of databus_single.py
import sys
import os
import serial
...
class DataBusNode(Node):
    """Single ROS2 node that owns publishers, subscribers, params, and the DAS serial backend."""
    ...
    def __init__(self,
        ...
        if not tty_port and param_side in ('left', 'right'):
            tty_port = f"/dev/ttyDevice{param_side.capitalize()}"
        if not tty_port:
            tty_port = find_serial_port("ttyUSB", side=param_side or None)
        if not tty_port:
    ...
    def _send_encoder_loop(self):
        ...
        self.get_logger().debug("Encoder loop thread exiting")
...
def main(args=None):
    # Strip ROS-injected argv if any
